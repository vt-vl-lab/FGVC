...
class InpaintGenerator(BaseNetwork):
    def __init__(self, config, residual_blocks=8, init_weights=True):
        ...
        self.config = config
        if config.FLO == 1:
            if config.PASSMASK == 0:
                in_channels = 3
            elif config.PASSMASK == 1:
                in_channels = 4
            else:
                assert(0)
            out_channels = 2
    ...
    def forward(self, input):
        ...
        if self.config.FLO == 0:
            x = (torch.tanh(x) + 1) / 2
        elif self.config.FLO == 1 and self.config.NORM == 1:
            if self.config.RESIDUAL == 1:
                assert(self.config.FILL == 1)
                x = torch.tanh(x + input[:, :2, :, :])
